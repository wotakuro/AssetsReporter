...
namespace UTJ.AssetsReporter
{
	...
	{
		private const string ResourceVersion = "20200529";


		public const string DestParentDir = "AssetsReporter";
		public const string ResultDir = DestParentDir + "/result/";
		public const string PreviewDir = DestParentDir + "/result/preview/";
		private const string VersionFile = DestParentDir + "/version.txt";
		...
		public static void UnzipPackageData(){
			if (IsAlreadyInstalled())
			{
				return;
			}
			string packageFilePath = "Packages/com.utj.assetsreporter/ReporterResource";
			int packageFilePathLength = packageFilePath.Length;
			var guids = AssetDatabase.FindAssets("", new string[] { packageFilePath });
			foreach( var guid in guids)
			{
				string path = AssetDatabase.GUIDToAssetPath(guid);
				string subPath = path.Substring(packageFilePathLength + 1);
				if(!subPath.Contains('.'))
				{
					continue;
				}
				string dest = DestParentDir  + '/' + subPath;
				...
				if(dest.EndsWith("excludeList.txt"))
				{
					if(File.Exists(dest))
					{
						continue;
			...
			File.WriteAllText(VersionFile, ResourceVersion);
		}
